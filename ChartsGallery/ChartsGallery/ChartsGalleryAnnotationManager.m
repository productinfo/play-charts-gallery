...
- (void)updateValueAnnotationForXAxisRange:(SChartRange *)xRange yAxisRange:(SChartRange *)yRange {
  ...
  for (int i = 0; i < self.lineAnnotationsLeft.count; ++i) {
    ...
    CGFloat height = [self.chart.yAxis pixelValueForDataValue:lineAnnotationLeft.maxYPosition] -
                     [self.chart.yAxis pixelValueForDataValue:lineAnnotationLeft.minYPosition];
